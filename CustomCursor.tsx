...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    // Where the real mouse is
    let targetX = window.innerWidth / 2;
    let targetY = window.innerHeight / 2;

    // Where the dot is currently drawn (eases toward target)
    let dotX = targetX;
    let dotY = targetY;
    ...
    const animate = () => {
      // Smoothly ease the dot toward the real cursor position
      ...
      // Spawn particles from the eased dot position, so the trail follows smoothly too
      for (let i = 0; i < 2; i++) {
        particles.push({
          x: dotX,
          y: dotY,
